...
Default: No. Offer the dashboard only after the report is complete. Generate it only when the user explicitly asks, with wording such as `dashboard yes`, `DASHBOARD=data`, or `DASHBOARD=html`.
...
Provide the smallest useful dashboard data object:
